...
const DeleteModal = ({show, onClose, text, isActivity = true, confirmHandler}) => {
    return (
        <>
            <div className={`absolute top-0 left-0 h-screen w-screen bg-black/50 md:bg-black/30 z-20 ${show ? 'block' : 'hidden'}`} onClick={onClose}>
            </div>
            <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-full w-full flex items-center justify-center z-20 ${show ? 'block' : 'hidden'}`} data-cy="modal-delete">
