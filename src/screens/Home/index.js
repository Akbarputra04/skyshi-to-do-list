    ...
    <>
			...
			<div className="min-h-screen px-5 pt-24 md:px-56 md:pt-36 pb-7 h-full flex flex-col gap-9">
				<div className="flex justify-between items-center">
					...
					<Button type="primary" icon={<BsPlusLg/>} text="Tambah" onClick={createActivity} data-cy="activity-add-button" />
				</div>
				{data.length > 0 ? (
					<div className="grid grid-cols-2 md:grid-cols-4 gap-5">
						{data.map(d => (
							<ActivityCard key={d.id} id={d.id} title={d.title} date={d.updated_at} setAlertShow={setAlertShow} setIsLoading={setIsLoading} data-cy="activity-item" />
						))}
					</div>
